...
import findConnectionPaths from "../lib/connection-paths";
import User from "../schemas/user.schema";
import { FriendshipGraph } from "../lib/types";
...
export const connectionPaths: RequestHandler = async (req: Request, res: Response): Promise<void> => {
    try {
        const user_id = req.user?.id;
        if (!user_id) {
            res.status(404).json({ message: "User not found" });
        ...
        const friendshipGraph: FriendshipGraph = {};
        allUsers.forEach((u) => {
            friendshipGraph[u.id] = u.friends.map((friend) => friend.id.toString());
        });

        const friendId = req.params.friendId as string;
        if (!friendshipGraph[friendId]) {
            res.status(404).json({ message: "Friend not found" });
            return;
        }

        const paths = findConnectionPaths(friendshipGraph, user_id, friendId);
